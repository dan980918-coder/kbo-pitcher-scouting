...
* **`{mlb,aaa}_fip_last` vs `{mlb,aaa}_fip_davenport_translated_last` — 서로 다른 걸 측정하는 별개 지표**: 전자(FanGraphs)는 **그 레벨 안에서의 상대적 위치**(그 시즌 그 레벨 투수들 사이에서 얼마나 잘했는지)이고, 후자(Clay Davenport, claydavenport.com)는 **그 성적을 MLB로 옮기면 어떻게 보일지의 추정치**(레벨 간 난이도 차이까지 반영해 변환한 값)다. "환산값이 항상 더 정확하다"는 뜻이 아니라 질문 자체가 다르다 — 스카우팅 관점에서는 오히려 둘을 같이 보는 게 유용하다(레벨에서는 평범했지만 환산하면 나쁘지 않은 선수, 반대로 레벨 성적은 좋았지만 환산하면 확 나빠지는 선수를 구분할 수 있음). AAA 151명 비교 기준 평균적으로 환산값이 더 나쁘게 나오는 경향이 있으나(레벨이 올라갈수록 어려워진다는 상식과 일치, mean diff +0.46), **개별 선수 단위로는 반대 방향으로 나오는 예외가 존재한다**(모집단 평균 패턴이지 모든 선수에게 적용되는 규칙이 아님) — 이는 결측이나 매칭 오류가 아니라 정상적인 분산의 일부로 취급한다.
* **Davenport 데이터의 동일 시즌·동일 레벨 다구단 처리**: claydavenport.com 개인 페이지는 FanGraphs MLB 피드의 `type==0` 같은 합계 행을 별도로 주지 않고, 시즌 중 트레이드로 같은 레벨에서 팀만 바뀐 경우도 팀별 행이 각각 나온다(예: 카일 하트 2023 AAA — Lehigh Valley 1.0이닝 + Tacoma 88.3이닝). 초기 구현에서 이걸 처리하지 않고 정렬 순서상 먼저 나온 행(1이닝짜리)을 그대로 "직전 시즌"으로 골라버려 `aaa_fip_davenport_translated_last`가 1.25라는 말이 안 되는 값이 나온 버그가 있었다(전체 패턴과 반대 방향으로 크게 튀어서 발견). Davenport 데이터는 raw count(HR/BB/K/IP)가 그대로 있어서 — FanGraphs 케이스와 달리 — **같은 (선수, 레벨, 시즌)의 모든 팀 스틴트를 raw count로 먼저 합산한 뒤 그 시즌 하나의 FIP를 계산**하도록 수정했다(수정 후 카일 하트 4.23으로 정상화, 이 버그로 부풀려졌던 다른 이상치들도 함께 해소됨 — `scripts/build_analysis_dataset.py`의 `load_davenport_season_rows` 참고).
